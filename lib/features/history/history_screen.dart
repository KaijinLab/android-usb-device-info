...
class _HistoryScreenState extends ConsumerState<HistoryScreen> {
  ...
  /// Build a "connection key" that uniquely identifies a device instance as best as we can.
  ///
  /// Priority:
  /// 1) VID:PID + serial (most stable)
  /// 2) VID:PID + deviceName (fallback)
  static String _connectionKey({
    ...
    // Prefer serial if available; otherwise fall back to deviceName.
      ...
      body: SafeArea(
        child: historyAsync.when(
          ...
          data: (items) {
            ...
            // Build a set of connected device keys (VID:PID + (serial|deviceName)).
            final connectedKeys = <String>{};
            final current = devicesAsync.asData?.value;
            if (current != null) {
              for (final it in current) {
                final d = it.device; // must match your existing model usage
                connectedKeys.add(
                  _connectionKey(
                    vendorId: d.vendorId,
                    ...
                    child: Row(
                      children: [
                        ...
                        FilledButton.tonalIcon(
                          onPressed:
                              items.isEmpty ? null : () => _confirmClearAll(context),
  ...
  String _titleFor(DeviceHistoryEntry e) {
    return e.productNameResolved ??
        e.productNameRaw ??
        (e.isInputDevice ? 'Input device' : 'USB device');
  }
  ...
  Future<void> _dismissOne(BuildContext context, DeviceHistoryEntry entry, int index) async {
    final removed =
        await ref.read(deviceHistoryControllerProvider.notifier).removeById(entry.id);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
          ...
          onPressed: () {
            ref
                .read(deviceHistoryControllerProvider.notifier)
                .restore(removed, index: index);
          },
        ),
      ),
    );
