...
def download_usb_ids(url: str, out_path: Path, timeout: int = 60) -> None:
    ...
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "usbdevinfo-usbids-updater/1.0 (+https://github.com/)",
            "Accept": "text/plain,*/*",
        },
    )
...
def build_sqlite_from_usb_ids(input_path: Path, output_db: Path, vacuum: bool) -> ParseState:
    ...
    try:
        # Creation performance. We will ship the DB read-only; still, avoid WAL artifacts.
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Fetch linux-usb usb.ids, build sqlite DB, write usbids.sql and usbids.sqlite.gz"
    )
    ap.add_argument(
        "--url",
    ...
    ap.add_argument(
        "--out-sql",
        default=None,
        help="Output SQL path (default: assets/db_src/usbids.sql)",
    )
    ap.add_argument(
        "--out-gz",
        default=None,
        help="Output gzip sqlite path (default: assets/db_src/usbids.sqlite.gz)",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="usbids_update_") as td:
        ...
        # Download (try HTTPS; if user passed HTTP, keep it)
        ...
        print(f"  - usbids.sqlite.gz   sha256={_sha256_file(out_gz)}")
